...
export function Experience() {
  return (
    <Section
      ...
      title={<>Training, <span className="text-gradient">open source</span> & hackathons.</>}
    >
      <div className="relative">
        ...
        <ul className="space-y-10">
          {items.map((it, i) => (
            ...
            >
              <div className={`pl-12 sm:pl-0 ${i % 2 === 0 ? "sm:text-right sm:pr-12" : "sm:[direction:ltr] sm:pl-12"}`}>
                ...
                <div className="font-mono text-xs text-muted-foreground">{it.when} · {it.tag}</div>
                <h3 className="mt-1 text-lg font-semibold">{it.title}</h3>
                <div className="text-sm text-muted-foreground">{it.where}</div>
              </div>
              <div className={`pl-12 sm:pl-0 ${i % 2 === 0 ? "sm:pl-12" : "sm:[direction:ltr] sm:text-right sm:pr-12"}`}>
